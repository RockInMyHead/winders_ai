...
async def generate_structured_response(messages: List[Dict], model: str, web_search_results: str = "") -> str:
    """
    Генерирует структурированный ответ используя подход "план - обдумывание - детализация"

    Процесс:
    1. Создание плана ответа
    2. Обдумывание и уточнение плана
    3. Детальная разработка каждого пункта плана
    4. Сбор финального ответа
    """
    from utils.openai_client import generate_response

    try:
        # ШАГ 1: Создание плана ответа
        plan_messages = [
            {
                "role": "system",
...
2. Разбей ответ на 3-7 логических разделов/пунктов
3. Каждый пункт должен быть конкретным и содержательным
4. План должен охватывать все аспекты вопроса
5. Используй нумерацию для пунктов плана
...
5. Заключение и выводы"""
            ...
            {"role": "user", "content": messages[-1]["content"]}  # Последнее сообщение пользователя
        ]
...
4. Предложи улучшения если необходимо

ВЫВОД: Краткий анализ плана и рекомендации по реализации."""
            },
            {"role": "user", "content": f"План ответа:\n{plan_response}\n\nВопрос пользователя: {messages[-1]['content']}\n\nПроанализируй план и дай рекомендации."}
        ]

        thinking_response = await generate_response(thinking_messages, model)
        print(f"🤔 АНАЛИЗ ПЛАНА:\n{thinking_response}")

        # ШАГ 3: Детальная разработка каждого пункта плана
        detailed_responses = []
